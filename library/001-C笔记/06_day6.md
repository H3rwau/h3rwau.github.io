...
strn系列函数包括strncpy,strncmp,strncat,当我们只想复制一部分源字符串的字符时，我们需要用strcpy,当只需要比较两个字符串的一部分是否相等时，我们使用strncmp,当需要将一个字符串的部分字符拼接到另一个字符串时，我们需要使用strncat。
...
| 函数名 | 定义                                                         | 功能                                                         |
| ------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| memset | void *memset(void *buffer,int ch,size_t count);              | 函数拷贝ch到buffer从头开始的count个字符里，并返回buffer指针memset()可以应用在将一段内存初始化为某个值。（可以将一个数组的所有元素设置成0） |
| memcpy | void *memcpy(void *to, const void *from,size_t count);       | 函数从from中复制count个字符到to中，并返回to指针。如果to和from重叠，则函数行为不确定。（当重叠时使用memmove) |

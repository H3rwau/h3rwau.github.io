...
OSI模型是一个用于计算机或通信系统间互联的标准体系，将计算机网络通信划分为七个不同的层级，每一层都负责特定的功能。且每一层构建在其下层之上，并为上层提供服务。这七层从下到上分别为物理层、数据链路层、网络层、传输层、会话层、表示层和应用层。但在实际网络通信中，TCP/IP模型更为实用。 它分为四个层级，分别为网络接口层，网络层，传输层和应用层。

1. 网络接口层对应OSI模型的数据链路层和物理层。它负责物理传输媒介的传输，例如以太网、Wi-Fi等，并提供错误检测和纠正的功能，还包含MAC地址的管理。

2. 网络层对应OSI模型的网络层。主要协议是IP协议，它负责数据包的路由和转发，选择最佳路径将数据从源主机传输到目标主机。IP协议使用IP地址来标识主机和网络，并进行逻辑地址寻址。

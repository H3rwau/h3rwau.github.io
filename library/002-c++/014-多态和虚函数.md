...
1. 封装可以隐藏实现细节，使得代码模块化；继承可以扩展已存在的代码
   模块（类）。它们的目的都是为了**代码重用**。而多态除了代码的复用性外，还可以
   **解决项目中紧偶合**的问题，**提高程序的可扩展性**。
...
3. C++支持两种多态性：**静态联编**（编译时多态）和**动态联编**（运行时多态）
   ...
   **编译时多态**：也称为**静态多态**，我们之前学习过的**函数重载**、**运算符重载**就是采用
...
1. 如果一个基类的成员函数定义为虚函数，那么**它在所有派生类中也保持为虚函数**，
...
1. 虚函数的实现是怎样的呢？简单来说，就是通过一张**虚函数表（ Virtual
   ...
   ![image-20241002172339847](D:\笔记gitpage\h3rwau.github.io\library\010-计算机八股\八股.assets\image-20241002172339847.png)
...
5. 成员函数中访问
   在**类内的成员函数中访问该类层次中的虚函数，采用动态联编，要使用 this 指针**。
...
​		的不同列表将同名的**函数的名称做修饰**，从而生成一些不同名称的预处理函数，
...
​		关键字）。
​	3.覆盖
​		**覆盖又可以叫做重写**，只有**重写了虚函数的才能算作是体现了C++多态性。**
​		基类存在某个虚函数，在派生类中对该虚函数**保持返回类型不变，参数类型个数**

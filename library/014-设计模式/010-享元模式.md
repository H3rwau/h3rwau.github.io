...
可以发现，棋盘的容器中存放了已下的棋子，而每个棋子包含棋子的所有属性。一盘棋往往需要含上百颗棋子，采用上面这种实现，占用的空间太大了。如何改进呢？用[享元模式](https://zhida.zhihu.com/search?content_id=184065615&content_type=Article&match_order=3&q=享元模式&zhida_source=entity)。其定义为：**运用共享技术有效地支持大量细粒度的对象**。
...
关注PieceBoard 的容器，之前是[vector](https://zhida.zhihu.com/search?content_id=184065615&content_type=Article&match_order=2&q=vector&zhida_source=entity)<Piece*> m_vecPiece，现在是vector<PiecePos> m_vecPos。这里是关键。
